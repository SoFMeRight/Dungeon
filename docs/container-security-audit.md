...
| MariaDB — plain (official image) | 5/5 | **Done** — 10 workloads (monica, erpnext, orangehrm, linkstack, photoprism, photoprism-x, bagisto, invoiceninja, roundcube [mysql:5.7], ghost) via uid-999 `/run/mysqld` + `/tmp` emptyDir recipe |
| MariaDB — galera (kimai, osticket) | 4/5 (RO N/A) | **Done** — container SC (`allowPrivilegeEscalation:false` + `drop:[ALL]`) on the MariaDB CR; RO omitted because `/run/mysqld` is on the rootfs, not an operator-mounted volume. GaleraReady stayed True through the rolling restart. |
| MariaDB — LSIO images (bookstack-db, dolibarr-mariadb, romm-db, shlink-db) | exception | **Documented exception** — `linuxserver/mariadb` runs as root via s6-overlay (needs writable rootfs + root for the PUID/PGID drop); non-root/RO not feasible. See the LinuxServer.io Images section. |
| Redis — harbor (Helm), immich/semaphore (multi-container) | — | **Deferred** — harbor via chart values; multi-ctr pods (immich, semaphore embedded postgres) need per-container review |
